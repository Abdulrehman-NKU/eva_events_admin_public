...
export default function DeleteExhibitor({
  title,
  description,
  user_id,
}: DeletePopoverProps) {
  ...
  const onClickDelete = async (id: string, setOpen: any) => {
    ...
    let res = await ExhibitorApi.deleteExhibitor(id);

    console.log('res-remove>>>', res);
    switch (res?.response_code) {
      case ResponseCodes.DELETE_SUCCESS:
        toast.success('Exhibitor Delete successfully.');
        toast.success('Exhibitors list reload in 1 sec.');
        setTimeout(() => {
          dispatch(usersAction?.setResetExhibitorsList());
        }, 1000);
        break;
