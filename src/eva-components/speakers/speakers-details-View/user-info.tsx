...
export default function UserInfo({
  ...
  speakersDetail: SpeakersDetailType | null;
}) {
  return (
    <>
      <div className="grid items-start gap-4 @xl:grid-cols-[80px_1fr] @2xl:grid-cols-[128px_1fr] md:gap-6 ">
        ...
        <UserProfilePhoto userData={speakersDetail as any} />
        <div className="grid grid-cols-2 gap-1 md:gap-1">
          <article>
            ...
            <p>
              <a href={`mailto:${speakersDetail?.email}`}>
                {speakersDetail?.email}
              </a>
            </p>
          </article>
          <article className="flex flex-wrap items-center justify-end gap-3">
            {speakersDetail?.phone && (
              <a
                href={`tel:+${speakersDetail?.phone_country_code}${speakersDetail?.phone}`}
              >
