...
const EditSpeakerForm = ({ editData }: { editData: SpeakersDetailType }) => {
  ...
  const initialValues = {
    ...
    linkedin: editData?.speaker_linkedin || '',
    url: editData?.speaker_URL || '',
    country: editData?.country || '',
    // state: editData?.first_name || "",
    city: editData?.city || '',
    // zip: editData?.zip || '',
    // addressLineOne: editData?.address_line_1 || '',
    // addressLineTwo: editData?.first_name || "",
  ...
  const onSubmitHandler = async (values: any) => {
    ...
    let res = await SpeakerApi.editSpeaker(formData, editData?._id);
