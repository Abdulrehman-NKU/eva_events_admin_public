...
export default function DeleteSpeaker({
  title,
  description,
  user_id,
}: DeletePopoverProps) {
  ...
  const onClickDelete = async (id: string, setOpen: any) => {
    ...
    let res = await SpeakerApi.deleteSpeaker(id);

    console.log('res-remove>>>', res);
    switch (res?.response_code) {
      case ResponseCodes.DELETE_SUCCESS:
        toast.success('Speaker Delete successfully.');
        toast.success('Speakers list reload in 1 sec.');
        setTimeout(() => {
          dispatch(usersAction?.setResetSpeakersList());
        }, 1000);
        break;
