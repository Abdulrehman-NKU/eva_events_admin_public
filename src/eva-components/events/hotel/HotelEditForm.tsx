...
const HotelCreateForm = ({ editData }: { editData: HotelListType }) => {
  const [state, setState] = useState<any>({
    profilePhoto: null,
    isLoading: false,
    selectedImages: [],
    editSelectedImages: editData?.hotel_images,
    drag: false,
  });
  ...
  const initialValues = {
    ...
    email: editData?.hotel_email || '',
    phoneNumber: editData?.phone || '',
    dialCode: editData?.phone_country_code || '',
    url: editData?.hotel_url || '',
  ...
  const hotelFormApi = async (values: any, poster_id_array: string[]) => {
    ...
    let update_poster_id_array = poster_id_array;
    if (state?.editSelectedImages?.length > 0) {
      let image_id_array = state?.editSelectedImages?.map(
        ({ _id }: { _id: string }) => {
          return _id;
        }
      );
      update_poster_id_array = poster_id_array.concat(image_id_array);
    }
    ...
    let formData = {
      ...
      hotel_images: update_poster_id_array,
    };

    let res = await HotelApi.editHotel(formData, editData?._id);
