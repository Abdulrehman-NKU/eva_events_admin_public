...
interface menuItemsDataType {
  label: string;
  type: string;
}

export default function ConferenceProgramNavigation({
  menuItemsData,
}: {
  menuItemsData: menuItemsDataType[];
}) {
  ...
  useEffect(() => {
    if (menuItemsData[0]?.label) {
      route.push(
        routes?.event?.conferenceProgramDateView(
          event_id,
      ...
      <div className="relative flex items-center overflow-hidden ">
        ...
        <div className="flex h-[52px] items-start overflow-hidden">
          <div
            ...
            ref={sliderEl}
          >
            {menuItemsData?.map(({ label, type }, index) => (
              <Link
                href={routes?.event?.conferenceProgramDateView(event_id, label)}
                key={`menu-${index}`}
                className={cn(
