...
export default function EventsHeaderNav() {
  ...
  let detailViewEventId = useSelector(
    (state: RootState) => state?.event?.detailViewEventId
  );
  ...
  useEffect(() => {
    ...
    if (!state.eventId) {
      dispatch(eventAction.setDetailViewEventId({ id: pathnameArray[2] }));
      setState({
        ...state,
        eventId: pathnameArray[2],
      });
    }
  }, []);
  return (
    ...
    >
      <div className="relative flex items-center overflow-hidden">
        <Button
          ...
          className="!absolute left-0 top-0.5 z-10 !h-[calc(100%-4px)] w-8 !justify-start bg-gradient-to-r from-white via-white to-transparent px-0 text-gray-500 hover:text-black  dark:from-gray-50 dark:via-gray-50"
        >
          ...
          >
            {menuItems.map(({ label, type }, index) => (
              <Link
                href={
                  type === 'exhibitor-info'
                    ? `/events/${state?.eventId}/exhibitor-info?info=general-information`
                    : routes?.event?.eventDetailView(state?.eventId, type)
                }
                key={`menu-${index}`}
                className={cn(
        ...
        <Button
          ...
          className="!absolute right-0 top-0.5 z-10 !h-[calc(100%-4px)] w-8 !justify-end bg-gradient-to-l from-white via-white to-transparent px-0 text-gray-500 hover:text-black  dark:from-gray-50 dark:via-gray-50"
        >
