...
import DeleteNetworkingEventModal from './DeleteNetworkingEventModal';
...
export default function NetworkingEventDetails() {
  ...
  let getEventDetailsApi = async () => {
    ...
    let networking_event_id = pathnameArray[5];
    setState({
      ...state,
      isLoading: true,
    });
    let res =
      await NetworkingEventApi.getNetworkingEventById(networking_event_id);

    console.log('res-networking_event_id>>>', res);
    ...
    <>
      {state?.isLoading ? (
        ...
        <>
            ...
            <div className="">
              ...
              >
                <NetworkingMetricCard
                  key={`${state?.eventDetail?._id}+${'location'}`}
                  title="Location"
                  metric={state?.eventDetail?.location}
                  icon={<TfiLocationPin className="h-6 w-6" />}
                ...
                <NetworkingMetricCard
                  ...
                  metric={
                    <DetailsTime
                      startTime={state?.eventDetail?.start_time}
                      endTime={state?.eventDetail?.end_time}
                    />
                  }
              ...
              <div>
                {ReactHtmlParser(state?.eventDetail?.description || 'hello')}
              </div>
            </div>
          </div>
          ...
          <DeleteNetworkingEventModal event_id={state?.eventDetail?._id} />
        </>
      )}
    </>
  );
