...
export default function DeleteNetworkingEvent({
  title,
  description,
  id,
}: DeletePopoverProps) {
  ...
  const onClickDelete = async (id: string, setOpen: any) => {
    ...
    let res = await NetworkingEventApi.deleteNetworkingEvent(id);
     ;
    console.log('res-remove>>>', res);
    switch (res?.response_code) {
      case ResponseCodes.DELETE_SUCCESS:
        toast.success('Networking-Event Delete successfully.');
        toast.success('Networking-Event list reload in 1 sec.');
        setTimeout(() => {
          dispatch(eventAction.setResetNetworkingEventList());
        }, 1000);
        break;
