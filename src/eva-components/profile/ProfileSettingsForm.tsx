...
import { emailFormate, getImageUrl } from '../userDetails/userDetails';
...
const ProfileSettingsForm = ({ userData }: { userData: any }) => {
  const [state, setState] = useState<state>({
    isLoading: false,
    selectedPhoto: null,
    isLoadingRole: false,
    roleList: [],
    editProfilePhoto: userData?.profile_image || '',
    deletePhotoLoading: false,
  });
  const router = useRouter();
  const pathname = usePathname();

  let phone_number = `${userData?.phone_country_code}${userData?.phone}`;

  let updateList;
  if (userData?.roles?.length > 0) {
    updateList = userData?.roles?.map(
  ...
  const initialValues = {
    ...
    dialCode: userData?.phone_country_code || '',
  };
              ...
              {state?.editProfilePhoto && (
                <>
                  <div
                    ...
                    onClick={() => handleDeletePhoto(userData?._id)}
                  >
