...
export default function DeleteAdmins({
  ...
  const onClickDelete = async (id: string, setOpen: any) => {
    ...
    let res = await AdminApi.deleteAdmins(id);
     ;
    console.log('res-remove>>>', res);
    switch (res?.response_code) {
      case ResponseCodes.DELETE_SUCCESS:
        toast.success('Admin Delete successfully.');
        toast.success('Admins list reload in 1 sec.');
        setTimeout(() => {
          dispatch(usersAction?.setResetAdminsList());
        }, 1000);
        break;
        ...
        <div className="w-56 pb-2 pt-1 text-left rtl:text-right">
          ...
          <div className="flex items-center justify-end">
            <Button
              ...
              onClick={() => onClickDelete(admin_id, setOpen)}
            >
              Yes
            </Button>
            <Button
