...
export default function DeleteSponsor({
  ...
  const onClickDelete = async (id: string, setOpen: any) => {
    ...
    let res = await SponsorApi.deleteSponsor(id);

    console.log('res-remove>>>', res);
    switch (res?.response_code) {
      case ResponseCodes.DELETE_SUCCESS:
        toast.success('Sponsor Delete successfully.');
        toast.success('Sponsors list reload in 1 sec.');
        setTimeout(() => {
          dispatch(usersAction?.setResetSponsorsList());
        }, 1000);
        break;
