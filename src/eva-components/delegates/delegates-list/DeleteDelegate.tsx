...
export default function DeleteDelegate({
  title,
  description,
  user_id,
}: DeletePopoverProps) {
  ...
  const onClickDelete = async (id: string, setOpen: any) => {
    ...
    let res = await DelegateApi.deleteDelegate(id);

    console.log('res-remove>>>', res);
    switch (res?.response_code) {
      case ResponseCodes.DELETE_SUCCESS:
        toast.success('Delegate Delete successfully.');
        toast.success('Delegates list reload in 1 sec.');
        setTimeout(() => {
          dispatch(usersAction?.setResetDelegatesList());
        }, 1000);
        break;
