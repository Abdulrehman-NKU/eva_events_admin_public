...
const DelegatesForm: React.FC<IndexProps> = ({ slug, className }) => {
  ...
  const onSubmitHandler = async (values: any) => {
    ...
    let res = await DelegateApi.addDelegate(formData);
    ...
    switch (res?.response_code) {
      case ResponseCodes.CREATE_SUCCESS:
        ...
        if (eventId) {
          toast.success(
            `Delegate has been created and assigned to ${
  ...
  let formSubtitle = 'Delegate Info';

  if (eventId)
    formSubtitle = `Creating new delegate for event ${
      (state?.eventDetails as any)?.name ?? ''
    }`;
        ...
        <div className="mb-10  grid gap-7 divide-y divide-dashed divide-gray-200 border-t bg-white @2xl:gap-9 @3xl:gap-11 dark:bg-gray-50">
          ...
          <div className=" relative mb-20">
            ...
            <div className=" absolute left-0 mt-3">
              <Button
                variant="outline"
                className="col-span-full ml-auto w-a_uto"
              >
                <PiPlusBold className="me-2 h-4 w-4" />
                Create New Company
              </Button>
