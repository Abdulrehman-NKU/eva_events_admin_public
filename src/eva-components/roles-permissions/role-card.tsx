...
import { RoleListType } from '../type/userListType';
import { Button, Tooltip } from 'rizzui';
import TrashIcon from '@/components/icons/trash';
import PencilIcon from '@/components/icons/pencil';
import DeleteSponsor from '../sponsors/sponsors-list/DeleteSponsor';
...
const RoleCard = ({ data }: { data: RoleListType[] }) => {
  const { openModal } = useModal();

  return (
    <>
      {data.map((role: RoleListType) => (
        <div
          key={role?._id}
          className={cn('rounded-lg border border-gray-200 p-6')}
        >
          <header className="flex items-center justify-between gap-2">
            ...
            <div className="ml-2 flex h-full flex-col items-end justify-between	">
              ...
              <DeleteSponsor
                ...
                description={`Are you sure you want to delete sponsor ?`}
                user_id={''}
              />
            </div>
          </header>
